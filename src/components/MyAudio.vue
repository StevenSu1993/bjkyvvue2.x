  ...
  <div style="width: 20%">
    ...
    <div class="audio-right">
      ...
      <div class="volume">
        ...
        <i class="iconfont icon-shengyin" :class="audioIcon" @click.stop="audioHuds = !audioHuds"> </i>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  directives: {
    ...
    adjuster: {
      inserted: function (el, binding, vnode) {
        el.addEventListener('click', (e) => {
          const hdiv = document.getElementById(vnode.context.volumeBarBg).clientHeight
          const audio = vnode.context.$refs.audioRef
          // 只有音乐开始播放后才可以调节，已经播放过但暂停了的也可以
          const ratemin = e.offsetY / hdiv
          const rate = ratemin * 100
          document.getElementById(vnode.context.volumeBar).style.height = rate + '%'
  ...
  mounted () {
    ...
    this.volumeBar = uuidByTime()
    this.volumeBarBg = uuidByTime()
    this.fetch()
  },
  // created () {
  //   this.progressBarBg = uuidByTime()
